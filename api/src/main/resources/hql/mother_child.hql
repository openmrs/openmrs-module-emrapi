...
where
  ...
  and (:childRequiredToBeBornDuringMothersActiveVisit = false or (select count(motherVisit) from Visit as motherVisit where motherVisit.patient = mother and motherVisit.stopDatetime is null and motherVisit.voided = false
                                                                                                                   and ((year(child.birthdate) > year(motherVisit.startDatetime))
                                                                                                                             or (year(child.birthdate) = year(motherVisit.startDatetime) and month(child.birthdate) > month(motherVisit.startDatetime))
                                                                                                                                or (year(child.birthdate) = year(motherVisit.startDatetime) and month(child.birthdate) = month(motherVisit.startDatetime) and day(child.birthdate) >= day(motherVisit.startDatetime)))) > 0)
