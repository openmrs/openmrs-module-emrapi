...
public class EmrApiActivator extends BaseModuleActivator implements DaemonTokenAware {
	...
	private void ensurePrivilegeLevelRoles() {
		...
		for (Privilege candidate : accountService.getApiPrivileges()) {
			...
			if (!highPrivilegeLevel.hasPrivilege(candidate.getName())
			        && !EmrApiConstants.UNSAFE_PRIVILEGES.contains(candidate.getName())) {
	...
	/**
	 * Creates missing mappings for metadata necessary for module functioning. MetadataTermMapping codes
	 * match global properties keys. Note that if mapping already has been created, and global property
	 * is edited, these changes will not be reflected in module metadata
	 * 
	 ...
	 * @should create new mapping with mapped object, if mapping is missing and there is global property
	 *         with uuid matching existing OpenmrsMetadata
	 */
	private void createMissingMetadataMappings() {
		//create main metadata source
		MetadataSource emrapiMetadataSource = saveMetadataSourceIfMissing(EmrApiConstants.EMR_METADATA_SOURCE_NAME,
		    EmrApiConstants.EMR_METADATA_SOURCE_DESCRIPTION);
		...
		GlobalPropertyToMappingConverter<EncounterRole> encounterRoleHelper = new GlobalPropertyToMappingConverter<EncounterRole>(
		        emrapiMetadataSource) {
		...
		GlobalPropertyToMappingConverter<EncounterType> encounterTypeHelper = new GlobalPropertyToMappingConverter<EncounterType>(
		        emrapiMetadataSource) {
		...
		GlobalPropertyToMappingConverter<Form> formHelper = new GlobalPropertyToMappingConverter<Form>(
		        emrapiMetadataSource) {
	...
	private void createExtraPatientIdTypesSetIfMissing(MetadataSource emrapiMetadataSource) {
		MetadataTermMapping extraPatientIdTypesMapping = metadataMappingService.getMetadataTermMapping(emrapiMetadataSource,
		...
		if (extraPatientIdTypesMapping == null) {
			...
			List<PatientIdentifierType> types = getPatientIdentifierTypesFromGlobalProperty(administrationService,
			    EmrApiConstants.GP_EXTRA_PATIENT_IDENTIFIER_TYPES);
			...
			extraPatientIdTypesMapping = new MetadataTermMapping(emrapiMetadataSource,
			        EmrApiConstants.GP_EXTRA_PATIENT_IDENTIFIER_TYPES, extraPatientIdTypesSet);
	 ...
	 */
	protected List<PatientIdentifierType> getPatientIdentifierTypesFromGlobalProperty(
	        AdministrationService administrationService, String gpKey) {
		...
		if (StringUtils.isNotEmpty(globalProperty)) {
			for (String type : globalProperty.split(",")) {
				PatientIdentifierType patientIdentifierType = Context.getPatientService()
				        .getPatientIdentifierTypeByUuid(type.trim());
				if (patientIdentifierType != null) {
					types.add(patientIdentifierType);
				} else {
					log.warn("Global property " + EmrApiConstants.GP_EXTRA_PATIENT_IDENTIFIER_TYPES
					        + " specifies an unknown patient identifier type: " + type);
				}
			}
		}
		return types;
	...
	private void createUnknownProvider() {
		...
		MetadataTermMapping mapping = metadataMappingService.getMetadataTermMapping(emrSource,
		...
		if (unknownProvider == null) {
			...
			GlobalProperty emrApiUnknownProviderUuid = administrationService
			        .getGlobalPropertyObject(EmrApiConstants.GP_UNKNOWN_PROVIDER);
			GlobalProperty coreApiUnknownProviderUuid = administrationService
			        .getGlobalPropertyObject("provider.unknownProviderUuid"); // there's an OpenMRS constant for this, but it isn't available until 1.10
			...
			if (unknownProvider == null) {
				if (coreApiUnknownProviderUuid != null
				        && StringUtils.isNotBlank(coreApiUnknownProviderUuid.getPropertyValue())) {
	...
	/**
	 * (public so that it can be used in tests, but you shouldn't use this in production code) Creates a
	 * single ConceptSource which we will use to tag concepts relevant to this module
	 */
