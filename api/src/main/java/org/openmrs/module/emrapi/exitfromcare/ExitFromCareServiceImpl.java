...
public class ExitFromCareServiceImpl extends BaseOpenmrsService implements ExitFromCareService {
	...
	public void closePatientPrograms(Patient patient, Concept outcome, Date completionDate) {
		...
		for (PatientProgram patientProgram : programWorkflowService.getPatientPrograms(patient, null, null, null, null, null,
		    false)) {
			if (patientProgram.getActive() && programWorkflowService
			        .getPossibleOutcomes(patientProgram.getProgram().getProgramId()).contains(outcome)) {
				patientProgram.setOutcome(outcome);
				patientProgram.setDateCompleted(completionDate.after(patientProgram.getDateEnrolled()) ? completionDate
				        : patientProgram.getDateEnrolled()); // if completion date is before date enrolled, just use completion date
